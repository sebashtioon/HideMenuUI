...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, PriceLabel> : ModifyBase<ModifyDerive<Der, PriceLabel>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(PriceLabel, create, , int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(PriceLabel, init, , int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(PriceLabel, setColor, , cocos2d::ccColor3B)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(PriceLabel, setOpacity, , float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(PriceLabel, setPrice, , int)
		}
	};
}
