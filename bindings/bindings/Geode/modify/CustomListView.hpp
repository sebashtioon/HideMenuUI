...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, CustomListView> : ModifyBase<ModifyDerive<Der, CustomListView>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0xa8600, Default, CustomListView, create, cocos2d::CCArray*, TableViewCellDelegate*, float, float, int, BoomListType, float)
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0xaa280, Thiscall, CustomListView, reloadAll, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0xa9bb0, Thiscall, CustomListView, setupList, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(CustomListView, getListCell, , char const*)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0xa94f0, Thiscall, CustomListView, loadCell, TableViewCell*, int)
		}
	};
}
