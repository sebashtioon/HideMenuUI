...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, AchievementNotifier> : ModifyBase<ModifyDerive<Der, AchievementNotifier>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x39cd0, Default, AchievementNotifier, sharedState, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(AchievementNotifier, achievementDisplayFinished, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x39db0, Thiscall, AchievementNotifier, notifyAchievement, char const*, char const*, char const*, bool)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x39e90, Thiscall, AchievementNotifier, showNextAchievement, )
