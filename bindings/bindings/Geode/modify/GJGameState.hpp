...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, GJGameState> : ModifyBase<ModifyDerive<Der, GJGameState>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x1f63a0, Thiscall, GJGameState, processStateTriggers, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x239b20, Thiscall, GJGameState, stopTweenAction, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x1f6250, Thiscall, GJGameState, tweenValue, float, float, int, float, int, float, int, int)
